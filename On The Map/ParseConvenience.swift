...
extension ParseClient {
    ...
    func getStudentLocations(completionHandler: (success: Bool, result: [StudentLocations]?, error: NSError?) -> Void) -> NSURLSessionDataTask? {
        ...
        let parameters: [String:AnyObject] = [ParseClient.JSONBodyKeys.Limit: ParseClient.Constants.Number, ParseClient.JSONBodyKeys.Order: ParseClient.Constants.OrderFormat]
    ...
    func postStudentLocations(mapString: String, mediaURL: String, latitude: Double, longitude: Double, completionHandler: (Success: Bool, createdAt: String?, error: NSError?) -> Void)  {
        ...
        /* 2. Make the request */
        taskForPOSTMethod(ParseClient.Methods.StudentLocation, parameters: parameters, jsonBody: jsonBody) { JSONResult, error in
